...
if not exist bin\mysql\bin\mysql.exe (
	...
	(
		echo. | set /p v="Installing MySQL 8.0.27..."^
		&& powershell -Command "$progressPreference = 'silentlyContinue'; Invoke-WebRequest -Method Get -Uri 'https://dev.mysql.com/get/Downloads/MySQL-8.0/mysql-8.0.27-winx64.zip' -OutFile '%CD%\temp\mysql.zip';" >nul 2>&1^
		...
		&& move temp\mysql\mysql-8.0.27-winx64 bin\mysql >nul 2>&1^
		&& echo SUCCESS
	) || (
		echo FAILED & goto quit
	)
...
if not exist bin\composer\composer.phar (
	...
	(
		echo. | set /p v="Installing Composer 2.2.4..."^
		&& powershell -Command "$progressPreference = 'silentlyContinue'; Invoke-WebRequest -Method Get -Uri 'https://getcomposer.org/download/2.2.4/composer.phar' -OutFile '%CD%\bin\composer\composer.phar';" >nul 2>&1^
		&& echo SUCCESS
	) || (
		echo FAILED & goto quit
	)
...
if not exist pma/index.php (
	...
	(
		echo. | set /p v="Installing phpMyAdmin 5.1.1..."^
		&& powershell -Command "$progressPreference = 'silentlyContinue'; Invoke-WebRequest -Method Get -Uri 'https://files.phpmyadmin.net/phpMyAdmin/5.1.1/phpMyAdmin-5.1.1-all-languages.zip' -OutFile '%CD%\temp\pma.zip';" >nul 2>&1^
		...
		&& move temp\pma\phpMyAdmin-5.1.1-all-languages pma >nul 2>&1^
		&& echo SUCCESS
	) || (
		echo FAILED & goto quit
	)
