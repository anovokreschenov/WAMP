...
if not exist pma/index.php (
	...
	(
		echo. | set /p v="Installing phpMyAdmin 5.1.3..."^
		&& powershell -Command "$progressPreference = 'silentlyContinue'; Invoke-WebRequest -Method Get -Uri 'https://files.phpmyadmin.net/phpMyAdmin/5.1.3/phpMyAdmin-5.1.3-all-languages.zip' -OutFile '%CD%\temp\pma.zip';" >nul 2>&1^
		...
		&& move temp\pma\phpMyAdmin-5.1.3-all-languages pma >nul 2>&1^
		&& echo SUCCESS
	) || (
		echo FAILED & goto quit
	)
