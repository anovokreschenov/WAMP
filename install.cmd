...
if not exist bin\mysql\bin\mysql.exe (
	...
	(
		echo. | set /p v="Installing MySQL 8.0.36..."^
		&& powershell -Command "$progressPreference = 'silentlyContinue'; Invoke-WebRequest -Method Get -Uri 'https://dev.mysql.com/get/Downloads/MySQL-8.0/mysql-8.0.36-winx64.zip' -OutFile '%CD%\temp\mysql.zip';" >nul 2>&1^
		...
		&& move temp\mysql\mysql-8.0.36-winx64 bin\mysql >nul 2>&1^
		&& echo SUCCESS
	) || (
		echo FAILED & goto quit
	)
...
if not exist bin\composer\composer.phar (
	...
	(
		echo. | set /p v="Installing Composer 2.7.1..."^
		&& powershell -Command "$progressPreference = 'silentlyContinue'; Invoke-WebRequest -Method Get -Uri 'https://getcomposer.org/download/2.7.1/composer.phar' -OutFile '%CD%\bin\composer\composer.phar';" >nul 2>&1^
		&& echo SUCCESS
	) || (
		echo FAILED & goto quit
	)
