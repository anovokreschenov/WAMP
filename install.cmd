...
if not exist bin\apache\bin\httpd.exe (
	...
	(
		echo. | set /p v="Installing Apache 2.4.52..."^
		&& powershell -Command "$progressPreference = 'silentlyContinue'; Invoke-WebRequest -Method Get -Uri 'https://home.apache.org/~steffenal/VC15/binaries/httpd-2.4.52-win64-VC15.zip' -OutFile '%CD%\temp\apache.zip';" >nul 2>&1^
...
if not exist bin\mysql\bin\mysql.exe (
	...
	(
		echo. | set /p v="Installing MySQL 8.0.28..."^
		&& powershell -Command "$progressPreference = 'silentlyContinue'; Invoke-WebRequest -Method Get -Uri 'https://dev.mysql.com/get/Downloads/MySQL-8.0/mysql-8.0.28-winx64.zip' -OutFile '%CD%\temp\mysql.zip';" >nul 2>&1^
		...
		&& move temp\mysql\mysql-8.0.28-winx64 bin\mysql >nul 2>&1^
		&& echo SUCCESS
	) || (
		echo FAILED & goto quit
	)
...
if not exist bin\php\php.exe (
	...
	(
		echo. | set /p v="Installing PHP 7.4.27..."^
		&& powershell -Command "$progressPreference = 'silentlyContinue'; Invoke-WebRequest -Method Get -Uri 'https://windows.php.net/downloads/releases/php-7.4.27-Win32-vc15-x64.zip' -OutFile '%CD%\temp\php.zip';" >nul 2>&1^
...
if not exist bin\composer\composer.phar (
	...
	(
		echo. | set /p v="Installing Composer 2.2.5..."^
		&& powershell -Command "$progressPreference = 'silentlyContinue'; Invoke-WebRequest -Method Get -Uri 'https://getcomposer.org/download/2.2.5/composer.phar' -OutFile '%CD%\bin\composer\composer.phar';" >nul 2>&1^
		&& echo SUCCESS
	) || (
		echo FAILED & goto quit
	)
...
if not exist bin\node\node.exe (
	if exist bin\node rd /q /s bin\node >nul 2>&1
	(
		echo. | set /p v="Installing Node.js 16.13.2..."^
		&& powershell -Command "$progressPreference = 'silentlyContinue'; Invoke-WebRequest -Method Get -Uri 'https://nodejs.org/dist/v16.13.2/node-v16.13.2-win-x64.zip' -OutFile '%CD%\temp\node.zip';" >nul 2>&1^
		&& powershell -Command "$progressPreference = 'silentlyContinue'; Expand-Archive -Path '%CD%\temp\node.zip' -DestinationPath '%CD%\temp\node' -Force;" >nul 2>&1^
		&& move temp\node\node-v16.13.2-win-x64 bin\node >nul 2>&1^
		&& echo SUCCESS
	) || (
		echo FAILED & goto quit
	)
...
if not exist pma/index.php (
	...
	(
		echo. | set /p v="Installing phpMyAdmin 5.1.2..."^
		&& powershell -Command "$progressPreference = 'silentlyContinue'; Invoke-WebRequest -Method Get -Uri 'https://files.phpmyadmin.net/phpMyAdmin/5.1.2/phpMyAdmin-5.1.2-all-languages.zip' -OutFile '%CD%\temp\pma.zip';" >nul 2>&1^
		...
		&& move temp\pma\phpMyAdmin-5.1.2-all-languages pma >nul 2>&1^
		&& echo SUCCESS
	) || (
		echo FAILED & goto quit
	)
