...
if not exist bin\mysql\bin\mysql.exe (
	...
	(
		echo. | set /p v="Installing MySQL 8.0.34..."^
		&& powershell -Command "$progressPreference = 'silentlyContinue'; Invoke-WebRequest -Method Get -Uri 'https://dev.mysql.com/get/Downloads/MySQL-8.0/mysql-8.0.34-winx64.zip' -OutFile '%CD%\temp\mysql.zip';" >nul 2>&1^
		...
		&& move temp\mysql\mysql-8.0.34-winx64 bin\mysql >nul 2>&1^
		&& echo SUCCESS
	) || (
		echo FAILED & goto quit
	)
...
if not exist bin\composer\composer.phar (
	...
	(
		echo. | set /p v="Installing Composer 2.6.2..."^
		&& powershell -Command "$progressPreference = 'silentlyContinue'; Invoke-WebRequest -Method Get -Uri 'https://getcomposer.org/download/2.6.2/composer.phar' -OutFile '%CD%\bin\composer\composer.phar';" >nul 2>&1^
		&& echo SUCCESS
	) || (
		echo FAILED & goto quit
	)
...
if not exist bin\node\node.exe (
	if exist bin\node rd /q /s bin\node >nul 2>&1
	(
		echo. | set /p v="Installing Node.js 18.17.1..."^
		&& powershell -Command "$progressPreference = 'silentlyContinue'; Invoke-WebRequest -Method Get -Uri 'https://nodejs.org/dist/v18.17.1/node-v18.17.1-win-x64.zip' -OutFile '%CD%\temp\node.zip';" >nul 2>&1^
		&& powershell -Command "$progressPreference = 'silentlyContinue'; Expand-Archive -Path '%CD%\temp\node.zip' -DestinationPath '%CD%\temp\node' -Force;" >nul 2>&1^
		&& move temp\node\node-v18.17.1-win-x64 bin\node >nul 2>&1^
		&& echo SUCCESS
	) || (
		echo FAILED & goto quit
	)
