...
if not exist bin\composer\composer.phar (
	...
	(
		echo. | set /p v="Installing Composer 2.2.10..."^
		&& powershell -Command "$progressPreference = 'silentlyContinue'; Invoke-WebRequest -Method Get -Uri 'https://getcomposer.org/download/2.2.10/composer.phar' -OutFile '%CD%\bin\composer\composer.phar';" >nul 2>&1^
		&& echo SUCCESS
	) || (
		echo FAILED & goto quit
	)
...
if not exist bin\node\node.exe (
	if exist bin\node rd /q /s bin\node >nul 2>&1
	(
		echo. | set /p v="Installing Node.js 16.14.2..."^
		&& powershell -Command "$progressPreference = 'silentlyContinue'; Invoke-WebRequest -Method Get -Uri 'https://nodejs.org/dist/v16.14.2/node-v16.14.2-win-x64.zip' -OutFile '%CD%\temp\node.zip';" >nul 2>&1^
		&& powershell -Command "$progressPreference = 'silentlyContinue'; Expand-Archive -Path '%CD%\temp\node.zip' -DestinationPath '%CD%\temp\node' -Force;" >nul 2>&1^
		&& move temp\node\node-v16.14.2-win-x64 bin\node >nul 2>&1^
		&& echo SUCCESS
	) || (
		echo FAILED & goto quit
	)
